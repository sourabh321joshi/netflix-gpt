  ...
  return (
    <div>
      ...
      className='w-3/12 absolute p-12 bg-black my-36 mx-auto right-0 left-0 text-white rounded-lg bg-opacity-85'>
       ...
       {!isSignInForm && <input ref={name} type='text' placeholder='Name' className='p-2 my-4 w-full bg-gray-800 '></input> }
        <input 
        ref={email} type='text' placeholder='Email Address' className='p-2 my-4 w-full bg-gray-800 '/>
        <input
         ref={password} type='password' placeholder='password' className='p-2 my-4 w-full bg-gray-800'/>
        ...
        <p 
        className='py-4 cursor-pointer' onClick={toggleSignInForm}>{isSignInForm ?"New to Netflix? Sign Up Now" :"Already a user Sign In Now"}</p>
      </form>
    </div>
  )
};
